...
rule all:
	input:
		#expand("analyses/seqbuster/count_tables/{gene}.count.table.csv",samplegroup=set(directories),gene=gene)
		expand("analyses/seqbuster_new/result/{sample}.truncated_trimmed.mirna",zip,samplegroup=directories,sample=files)
...
rule adapter_trimming:
	...
	output:
		"data/trimmed/{sample}.truncated.fastq.gz"
...
rule seqcluster_seqbuster:
	...
	params:
		...
		mirna="~/data/miRBase/miRNA.str",
		hairpin="~/data/miRBase/hsa-hairpin.fasta",
		sps="hsa"
...
rule create_seqbuster_count_tables:
	...
	output: 
		"analyses/seqbuster/result/{sample}.truncated_trimmed.mirna.hairpincounts.tsv",
		"analyses/seqbuster/result/{sample}.truncated_trimmed.mirna.mirnacounts.tsv",
		"analyses/seqbuster/result/{sample}.truncated_trimmed.mirna.isomircounts.tsv"
...
rule create_final_seqbuster_tables:
	input:
                lambda wildcards : ["analyses/seqbuster/result/" + x + ".truncated_trimmed.mirna." + wildcards.gene + "counts" + ".tsv" for x in file_dict[wildcards.samplegroup]]

	output:
		"analyses/seqbuster/count_tables/{gene}.count.table.csv"

	shell:
		'''
		generic_table_creator.R {wildcards.gene}counts analyses/seqbuster/result/
                
		mv analyses/seqbuster/result/{wildcards.gene}counts.count.table.csv analyses/seqbuster/count_tables/{wildcards.gene}.count.table.csv
